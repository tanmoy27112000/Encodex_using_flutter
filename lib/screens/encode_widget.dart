...
class encodeWidget extends StatefulWidget {
  encodeWidget({Key key}) : super(key: key);

  _encodeWidgetState createState() => _encodeWidgetState();
}

class _encodeWidgetState extends State<encodeWidget> {
  var charList = [
    'A',
    'B',
    'C',
  ...
  void _encrypt() {
    ...
    var textList = new List(text.length);
    for (int i = 0; i < text.length; i++) {
      textList[i] = text[i];
    }
    for (int i = 0; i < text.length; i++) {
      String st = textList[i];
      for (int j = 0; j < charList.length; j++) {
        if (st == charList[j]) {
          index = j;
          textList[i] = codeList[index];
          break;
        }
      }
    }
    for (int i = 0; i < text.length; i++) {
      //print(textList[i]);
      if (i % 2 == 0) {
        textList[i] = textList[i] + 5;
      } else {
        textList[i] = textList[i] + 3;
      }
    }

    for (int i = 0; i < text.length; i++) {
      encodedText = encodedText + textList[i].toString() + " ";
    }
    //debugPrint(encodedText);
    if (password.length == 0) {
      encodedText = "Password makes your text protected!!";
    }
  }
}
